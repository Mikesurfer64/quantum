...
def plot_mandlebrot_set(ss):
    ...
    for sy in reversed(range(ss.screen_height)):
        ...
        for sx in range(ss.screen_width):
            wx = ss.world_x(sx)
            zx, zy = wx, wy #Define relations for mandelbrot set below 
            zx_2, zy_2 = zx * zx, zy * zy #""
            iter = 0
            while zx_2 + zy_2 < radius and iter < max_iter: #while loop rather than a for loop to evaluate close to infinite values
                nx = zx_2 - zy_2 + wx                       
                ny = 2 * zx * zy + wy
                zx = nx
                zy = ny
                ...
                zy_2 = zy * zy
                iter += 1
            # Select color using HSV encoding, HSV encoding used more widely than RGB
...
def main():
    ss = SimpleScreen( #define rectangle 
