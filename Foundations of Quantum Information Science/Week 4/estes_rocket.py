...
STAGE2_MASS = 0.1729  # kg mass of booster motor
...
def thrust_func(t):
    ...
    if t < ENGINE_BURNOUT * 2: #1.6 s to burnout, 2 for stages of rocket
...
def rocket_mass_func(t):
    ...
    if t <= ENGINE_BURNOUT:
        motor_mass = ENGINE_MASS * (ENGINE_BURNOUT - t) / ENGINE_BURNOUT #linear burnrate
...
def model(t, state_vector): 
    v, h = state_vector # v= velocity, h=height
    m = rocket_mass_func(t) #changing as a function of t
    F_thrust = thrust_func(t) #""
    F_gravity = G * M * m / (R + h) ** 2
    d_v = (F_thrust - F_gravity) / m #F=ma
    d_h = v #s= vt
    return d_v, d_h
...
def main():
    ...
    t = sol.t
    v, h = sol.y #tuple of vectors (v= vectorized)
